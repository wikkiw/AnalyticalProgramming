...
public class CMAEvolutionStrategy implements java.io.Serializable {
    ...
    public void setCustomGenerator(AP.util.random.Random rndGenerator) {
    		...
    		if (typicalX != null) {
    			...
    			for (i = 0; i < N; ++i)
    				//xmean[i] += sigma*diagD[i] * rand.nextGaussian();
                                xmean[i] += sigma*diagD[i] *OtherDistributionsUtil.normal(0, 1);
    			...
    			for (i = 0; i < N; ++i) { /* TODO: reconsider this algorithm to set X0 */
    				...
    				//xmean[i] = LBound[i] + offset + rand.nextDouble() * range;
                                xmean[i] = LBound[i] + offset + rndGenerator.nextDouble() * range;
    			}
    		} else {
    			error("no initial search point (solution) X or typical X specified");
    			xmean = new double[N];
    			for (i = 0; i < N; ++i)
    				xmean[i] = rndGenerator.nextDouble();
    		}
    	}
        	...
        	if (flgdiag)  
        	    for (i = 0; i < N; ++i)
        			//arx[iNk][i] = xmean[i] + sigma * diagD[i] * rand.nextGaussian();
                                arx[iNk][i] = xmean[i] + sigma * diagD[i] * OtherDistributionsUtil.normal(0, 1);
        	else {
                for (i = 0; i < N; ++i) 
        		    //artmp[i] = diagD[i] * rand.nextGaussian();
                            artmp[i] = diagD[i] * OtherDistributionsUtil.normal(0, 1);
        ...
        if (flgdiag)
            for (i = 0; i < N; ++i)
        		//arx[index][i] = xmean[i] + sigma * diagD[i] * rand.nextGaussian();
                        arx[index][i] = xmean[i] + sigma * diagD[i] * OtherDistributionsUtil.normal(0, 1);
        else {
            for (i = 0; i < N; ++i) 
                //artmp[i] = diagD[i] * rand.nextGaussian();
                artmp[i] = diagD[i] * OtherDistributionsUtil.normal(0, 1);
...
/** search space dimensions must be set before the optimization is started. */
    ...
    public void setInitialX(double l, double u) {
    	...
    	for (int i = 0; i < xmean.length; ++i)
    		xmean[i] = l + (u-l) * rndGenerator.nextDouble();
    	lockDimension = 1;
    }
    ...
    public void setInitialX(double[] l, double[] u) {
    	...
    	xmean = new double[N];
    	for (int i = 0; i < xmean.length; ++i)
    		xmean[i] = l[i] + (u[i]-l[i]) * rndGenerator.nextDouble();
    	lockDimension = 1;
    }
